...
class LoggerClientTest {
    ...
    @Test
    void log_retriesAndSucceedsOnSecondAttempt() throws Exception {
        UUID id = UUID.randomUUID();
        LogResponse logResponse = new LogResponse(id.toString(), "APPROVED");
    ...
    @Test
    void log_returnsFalseAfterThreeFailures() {
        mockServer.expect(requestTo("http://localhost:8088/api/internal/log"))
                .andRespond(withStatus(HttpStatus.SERVICE_UNAVAILABLE));
        mockServer.expect(requestTo("http://localhost:8088/api/internal/log"))
                .andRespond(withStatus(HttpStatus.SERVICE_UNAVAILABLE));
